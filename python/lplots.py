...
sig_3tev[:,1] = sig_3tev[:,1]*4.4*5.0*100.0
sig_6tev[:,1] = sig_6tev[:,1]*12.0*5.0*100.0
sig_clic[:,1] = sig_clic[:,1]*8.0*5.0*100.0
print(sig_3tev)
print(sig_6tev)
print(sig_clic)
...
l2 = plt.legend([p3,p4],[lb3,lb4], loc=6)
plt.gca().add_artist(l1)

plt.ylim(ymin=0)

plt.xlabel("$\lambda / \lambda_{SM}$",horizontalalignment='right',fontsize=18)
plt.ylabel("# Events       ",verticalalignment='top',rotation='horizontal',fontsize=18)
plt.title("Two-Higgs Events vs. Higgs Self-Coupling")
